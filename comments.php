...
function checkCommentsAllow($type, $parentID){
	global $userID, $useraccessgroups;
	$moduls = array();
	...
	if(mysql_num_rows($get)){
		...
		$allowed=hasaccess($row['comments'], $useraccessgroups);
	}
	return $allowed;
}
...
elseif(isset($_POST['saveeditcomment'])) {
	...
	if($gesamt) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			if($ds['userID']) {
				...
				$avatar='<img src="images/avatars/'.getavatar($ds['userID']).'" alt="'.$_language->module['avatar'].'" />';
				...
				$member='';
				$avatar='<img src="images/avatars/noavatar.gif" alt="'.$_language->module['avatar'].'" />';
				$country='';
				$pm='';
				$buddy='';
				$statuspic='';
				...
				$quotemessage = str_replace("'", "`", $ds['comment']);
			...
			if(isfeedbackadmin($userID) or iscommentposter($userID,$ds['commentID'])) {
				$edit = '<input type="button" value="'.$_language->module['edit'].'" onclick="window.location=\'index.php?site=comments&amp;editcomment=true&amp;id='.$ds['commentID'].'&amp;ref='.urlencode($referer).'\'" />';
			} else $edit='';
	...
	if($comments_allowed && checkCommentsAllow($type,$parentID)) {
		if($loggedin) {
            eval ("\$addbbcode = \"".gettemplate("addbbcode")."\";");
		...
		if(!$comments_allowed){
			...
			echo $_language->module['comments_na_by_you'];
		}
	}
}
