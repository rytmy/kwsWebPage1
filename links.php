...
elseif($action=="show" AND is_numeric($_GET['linkcatID'])) {
	...
	if(mysql_num_rows($linkcat)) {
		...
		while($ds=mysql_fetch_array($linkcat)) {
			...
			if(ispageadmin($userID) || isnewsadmin($userID)) {
				$adminaction='<br /><br /><input type="button" onclick="MM_goToURL(\'parent\',\'index.php?site=links&amp;action=edit&amp;linkID='.$ds['linkID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
