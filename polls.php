...
if($action=="vote") {
	...
	if(isset($_POST['pollID']) && isset($_POST['vote'])){
	  ...
	  if(!isset($userID)) {
		...
		if(isset($_COOKIE['poll'])) $cookie_poll = $_COOKIE['poll'];
		else $cookie_poll = null;
	  if($cookie_poll != $pollID and !$anz and !$anz_user and isset($_POST['vote'])) {
	
			//write cookie
			setcookie("poll", $pollID, time() + (3600 * 24 * 365));
...
elseif(isset($_GET['vote'])) {
	...
	if($anz) {
		...
		$cookie = (isset($_COOKIE['poll'])) ? $_COOKIE['poll'] : NULL;
		
		if($cookie == $ds['pollID'] or $anz or $anz_user)
