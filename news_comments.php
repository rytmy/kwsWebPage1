...
echo $post;
if(isnewsadmin($userID) OR isnewswriter($userID)) echo '<br /><br />';
...
if($newsID) {
	...
	if($ds['intern'] <= isclanmember($userID) && ($ds['published'] || (isnewsadmin($userID) || (isnewswriter($userID) and $ds['poster'] == $userID)))) {
		...
		$adminaction='';
		$adminspacer = '';
		if(isnewsadmin($userID) OR isnewswriter($userID)) $adminspacer='<br />';
		if(isnewsadmin($userID)) {
			if(!$ds['published']) $adminaction.='<form style="display: inline;" method="post" action="news.php?quickactiontype=publish"><input type="hidden" name="newsID[]" value="'.$ds['newsID'].'" /><input type="submit" name="submit" value="'.$_language->module['publish_now'].'" /></form> ';
			else $adminaction.='<form style="display: inline;" method="post" action="news.php?quickactiontype=unpublish"><input type="hidden" name="newsID[]" value="'.$ds['newsID'].'" /><input type="submit" name="submit" value="'.$_language->module['unpublish'].'" /></form> ';
		}
